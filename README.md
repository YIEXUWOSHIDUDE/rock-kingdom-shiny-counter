...
安装器会在解压前自动检测 Windows 版本、系统位数、安装盘空间、NVIDIA 显卡、显存、驱动版本和 CUDA 计算能力。绿色项目表示通过；黄色表示系统无法可靠读取该信息，允许继续安装并在首次启动时复核；红色表示明确不满足最低要求，安装按钮会保持禁用。检测优先使用 NVIDIA 官方的 `nvidia-smi`，不可用时使用 Windows WMI 兜底，避免因为读取不到信息直接误判。
...
测试覆盖 OCR 关键词匹配、跨文字框匹配、结算画面去重、重新布防、保底状态、位置锁定设置、每轮次数记录、浅色对话框文字对比度、快捷键默认关闭、旧快捷键迁移、穿透恢复保护、托盘恢复、撤销、重置、原子保存和 ZIP 导入导出。Windows 截屏、CUDA、真实快捷键注册、系统托盘、声音和真实游戏画面仍需在目标电脑上人工验收。
